        ...
        <section class="login-section">
            <div class="wrapper">
                ...
                <div class="account-box">
                    <div class="form">
                        <!--右对齐 :model="form"：绑定的表单数据模型。:rules="rules"：验证规则。-->
                        <!--ref 将表单组件实例引用存储到 formRef-->
                        <el-form label-position="right" label-width="60px" status-icon :model="form" :rules="rules" ref="formRef">
                            ...
                            <!--@click-->
                            <el-button size="large" class="subBtn" v-on:click="doLogin">点击登录</el-button>
                        </el-form>
                    </div>
                </div>
            </div>
...
<script setup>
    ...
    // formRef.value 存储了 <el-form> 组件实例
    ...
    const doLogin = ()=>{
        formRef.value.validate(async (valid)=>{
            ...
            if (valid){
                ...
                try {
                    const res = await useUStore.getUserInfo({account, password})
